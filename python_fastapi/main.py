...
async def initialize_item_list(items: List[str]):
    ...
    if not items:
        raise HTTPException(
            status_code=400, detail="Provided item list cannot be empty."
        )
...
async def get_items(img_path: str):
    ...
    for box, text, confidence in result:
        ...
        if float(confidence) > 0.4 and text_upper not in [
            "SEARCH...",
    ...
    for item in grouped_items:
        ...
        if "prime" in ocr_lower:
            ...
            search_candidates = [
                x for x in search_candidates if x.lower().startswith(first_char)
            ]
        # -------------------------------
    ...
    return {"status": "success", "items": final_verified_items}
